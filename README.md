...
`app.js` 하나에 섞여 있던 걸 기능별 ES 모듈로 쪼갰습니다. `index.html`은 `main.js` 하나만 부르고,
`main.js`가 나머지를 `import` 합니다. (JSZip·XLSX는 CDN `<script>`로 먼저 로드된 전역을 그대로 씁니다.)

| 파일 | 하는 일 | export |
|---|---|---|
| `config.js` | 공용 상수 (`COLUMNS` 10개 열, `AI_COLUMNS`, `AI_ENDPOINT`=`/gemini`) | `COLUMNS`, `AI_COLUMNS`, `AI_ENDPOINT` |
| `pptx.js` | pptx에서 슬라이드 글자 읽기 (`slidePaths`, `readSlideText`, `readSlides`) | `readSlides` |
| `ai.js` | 프롬프트 조립 + 서버 함수 호출 (`buildPrompt`, `askGemini`) | `askGemini` |
| `testcase.js` | AI 날것을 우리 열 규격으로 강제 (`toCell`, `toRow`, `toRows`) | `toRows` |
| `excel.js` | 엑셀(xlsx) 저장 (`saveExcel`) | `saveExcel` |
| `main.js` | `step` 헬퍼 + 버튼 연결 (읽기=기능1, 엑셀로 내보내기=파이프라인). `index.html`이 부르는 유일한 파일 | — |
| `functions/gemini.js` | 서버 함수. `/gemini` 주소를 만들고 Gemini 키를 서버에만 둔다 (`onRequestPost`, `generate`, `findUsableModel`) | `onRequestPost` |
...
2. 중간에 AI를 거쳐 칸이 채워진 `testcases.xlsx` 다운로드 (요구사항 1개당 정상·예외 TC 여러 줄)

### 키는 서버에 둔다

프론트(`ai.js`)는 Gemini를 직접 부르지 않고 `/gemini`(= `functions/gemini.js`)를 부릅니다.
키는 그 서버 함수에만 있고 브라우저로는 나가지 않습니다.
**키를 프론트 코드에 적으면 배포 순간 인터넷에 공개됩니다** (JS를 긁어 키를 찾는 스캐너가 상시로 돕니다).

로컬 실행 — `file://`로 열면 `/gemini`가 없어서 안 됩니다. 로컬 서버가 필요합니다:
...
함수마다 `console.log("[함수명]", 값)` 형식으로 입력과 결과를 찍어둬서, 어디서 터졌는지 바로 보입니다.
**로그가 뜨는 곳이 둘로 나뉩니다:**

1. 서버를 띄웁니다 (프로젝트 루트에서):

   ```bash
   npx wrangler pages dev .          # http://localhost:8788
   ```

2. **프론트 로그**(`config`/`pptx`/`ai`/`testcase`/`excel`/`main`)는 **브라우저 개발자도구 Console**에 뜹니다.
3. **서버 함수 로그**(`functions/gemini.js`의 `onRequestPost`/`generate`/`findUsableModel`)는 브라우저가 아니라
   `wrangler pages dev`를 띄운 **터미널**에 뜹니다.

## 건드리면 안 되는 것

1. **세미콜론을 지우지 말 것.** JS의 자동 세미콜론 삽입(ASI)이 `(` `[` 로 시작하는 줄에서
   앞 줄과 잘못 붙어 버그를 냅니다. 그래서 이 프로젝트는 세미콜론을 일부러 유지합니다.
2. **`functions/gemini.js`(및 `functions/` 폴더)를 지우지 말 것.** 이 파일이 `/gemini` 주소를 만들고
   Gemini 키를 "서버에만" 두는 곳입니다. 지우면 AI 변환이 통째로 죽고,
   키를 프론트로 옮기면 배포 순간 인터넷에 키가 노출됩니다.
